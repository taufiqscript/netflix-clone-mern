    ...
    return (
        <div className='relative bg-black w-full border-t-stone-800 border-t-8 p-8 text-white'>
            <div className='max-w-6xl mx-auto'>
                <h3 className='text-center text-xl sm:text-4xl font-bold'>{language == "en" ? FAQ_TITLE_EN : FAQ_TITLE_ID}
                </h3>
                <ul className='relative flex flex-col gap-2 py-6 sm:py-12'>
                    <EachUtils
                        of={language == "en" ? LIST_FAQ_EN : LIST_FAQ_ID}
                        render={(item, index) => (
                            <div key={index}>
                                <li className='bg-[#1e1e1e] hover:bg-[#2d2d2d] transition-all py-2 sm:py-6 px-4 cursor-pointer'>
                                    <button
                                        onClick={() => setOpenContentIndex(openContentIndex == index ? null : index)}
                                        className='text-sm sm:text-2xl text-left font-semibold cursor-pointer flex justify-between w-full items-center'
                                    >
                                        {item.title}
                                        <motion.span
                                ...
                                <motion.div
                                    ...
                                    className='bg-[#1e1e1e] mt-0.5 p-4 sm:py-6 sm:text-xl text-sm'>
                                    <p>{item.desc}</p>
                                </motion.div>
                            </div>
                        )}
